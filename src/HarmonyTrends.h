...
private:
    ...
    void initialize() {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            similarSongsText[i].setPosition(90, 245 + i * 30);
            ...
            similarArtistsText[i].setPosition(90, 415 + i * 30);
        ...
        //firstSongHold[0].setString("[Placeholder]");

        firstSongComp[1].setString("Bar Confidence");
        //firstSongHold[1].setString("[Placeholder]");

        firstSongComp[2].setString("Song Hotttnesss");
        //firstSongHold[2].setString("[Placeholder]");

        firstSongComp[3].setString("Loudness");
        //firstSongHold[3].setString("[Placeholder]");

        firstSongComp[4].setString("Mode Confidence");
        //firstSongHold[4].setString("[Placeholder]");
        ...
        //secondSongHold[0].setString("[Placeholder]");

        secondSongComp[1].setString("Bar Confidence");
        //secondSongHold[1].setString("[Placeholder]");

        secondSongComp[2].setString("Song Hotttnesss");
       // secondSongHold[2].setString("[Placeholder]");

        secondSongComp[3].setString("Loudness");
      //  secondSongHold[3].setString("[Placeholder]");

        secondSongComp[4].setString("Mode Confidence");
       // secondSongHold[4].setString("[Placeholder]");
    }
    ...
    void handleEvents() {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                ...
                if (searchButton.getGlobalBounds().contains(event.mouseButton.x, event.mouseButton.y)) { //search bar
                    resetState();
                    if (!inputString1.empty() && !inputString2.empty()) { //str 1 song, str 2 artist
                        Song search = findSong(songs, inputString2, inputString1); //search for song
                        cout << search.title << " " << search.artist_name << endl;
                        minHeap pq(10000);
                        vector<Song> diff = songs; // difference between quantifiers
                        for (auto& song : diff){
                            song.data = abs(song.data - search.data); //change to difference off the two
                        }
                        for (auto& song : diff){
                            if (song.artist_name != inputString2 && song.title != inputString1){ // if its unique push to pq
                                pq.push(song);
                            }
                        }
                        song1 = pq.pop();
